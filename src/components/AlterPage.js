...
function AlterPage({
  ...
  useEffect(() => {
    ...
    } else if (activeParameter === "color") {
      configTabSvgRef.current.firstChild.setAttribute("fill", "#ffffff");
      colorTabSvgRef.current.firstChild.setAttribute("fill", "#ffc700");
      for (var i = 0; i < strokeTabSvgRef.current.children.length; i++) {
        strokeTabSvgRef.current.children[i].setAttribute("fill", "#ffffff");
      }
    } else if (activeParameter === "stroke") {
      configTabSvgRef.current.firstChild.setAttribute("fill", "#ffffff");
      colorTabSvgRef.current.firstChild.setAttribute("fill", "#ffffff");
      for (var i = 0; i < strokeTabSvgRef.current.children.length; i++) {
        strokeTabSvgRef.current.children[i].setAttribute("fill", "#ffc700");
      }
    }
  }, [activeParameter]);
  useEffect(() => {
    ...
    setSliderInputScaleValue(
      round(((alterScaleValue - 20) * (100 - 0)) / (120 - 20) + 0)
    );
