...
function AlterPage({
  ...
  function handleSliderInput(e, type, min, max, round) {
    if (e.target.value > max) {
      setSliderInputFValue(max);
      `set${type}Value(max)`;
    } else if (e.target.value < min) {
      `setSliderInput${type}Value(min)`;
      `set${type}Value(min)`;
    } else {
      var temp = Math.round(e.target.value * round) / round;
      `setSliderInput${type}Value(temp)`;
      `set${type}Value(temp)`;
    }
  }
                    ...
                    onKeyDown={(e) => {
                      if (e.key === "Enter") {
                        if (e.target.value > 100) {
                          ...
                          setScaleValue(
                            20 + round(slope(0, 100, 20, 120) * (100 - 0))
                          );
                          ...
                          setScaleValue(
                            20 + round(slope(0, 100, 20, 120) * (temp - 0))
                          );
                        }
                      }
                    }}
